...
const DEFAULT_IMAGES: ImageItem[] = [
  { src: '/IMG_6816.JPG', alt: 'Abstract art' },
  { src: '/IMG_2907.JPG', alt: 'Modern sculpture' },
  { src: '/IMG_2888.JPG', alt: 'Digital artwork' },
  { src: '/IMG_2476.JPG', alt: 'Contemporary art' },
  { src: '/IMG_6927.JPG', alt: 'Geometric pattern' },
  { src: '/IMG_6922.JPG', alt: 'Textured surface' },
  { src: '/IMG_2019.JPG', alt: 'Social media image' },
];
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const close = () => {
      ...
      // Find overlay in document.body (where we append it now)
      const overlay = document.body.querySelector('.enlarge') as HTMLElement | null;
      if (!overlay) return;
  ...
  const openItemFromElement = (el: HTMLElement) => {
    ...
    const targetSize = Math.min(viewportWidth * 0.8, viewportHeight * 0.8, 600);
    ...
    // Trigger reflow then animate to center
    void overlay.offsetHeight;
    
    setTimeout(() => {
      if (!overlay.parentElement) return;
      overlay.style.opacity = '1';
      overlay.style.left = `${(viewportWidth - targetSize) / 2}px`;
      overlay.style.top = `${(viewportHeight - targetSize) / 2}px`;
        ...
        style={
          {
            ...
            ['--image-filter' as string]: grayscale
              ? 'grayscale(1)'
              : 'none',
          } as React.CSSProperties
        }
      >
        <div ref={mainRef} className="stage touch-none">
        ...
        <div ref={viewerRef} className="viewer-frame">
